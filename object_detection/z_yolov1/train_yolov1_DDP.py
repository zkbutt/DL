...
linux用这个   python -m torch.distributed.launch --nproc_per_node=2 /home/win10_sys/tmp/DL/object_detection/z_yolov1/train_yolov1_DDP.py
...
if __name__ == '__main__':
    ...
    '''---------------数据加载及处理--------------'''
    loader_train, loader_val_fmap, loader_val_coco, train_sampler = data_loader(model.cfg, is_mgpu=True)
    ...
    train_eval(start_epoch=start_epoch, model=model, optimizer=optimizer, lr_scheduler=lr_scheduler,
               ...
               device=device, train_sampler=train_sampler, tb_writer=tb_writer,
               )
