...
from f_tools.datas.f_map.fmap_api import draw_plot_func, error, file_lines_to_list, file_lines_to_list, \
    is_float_between_0_and_1, draw_text_in_image, voc_ap, log_average_miss_rate


def f_do_fmap(path_gt, path_dt, path_img, confidence=0.5,
              iou_map=[], ignore_classes=[], console_pinter=True,
              plot_res=False, animation=False):
    MINOVERLAP = confidence  # 置信度 default value (defined in the PASCAL VOC2012 challenge)

    '''自定义参数'''
    no_animation = not animation  # 是否动画
    no_plot = not plot_res  # 是否日志 和画图
    quiet = console_pinter  # 控制台输出
    set_class_iou = iou_map
    ignore = ignore_classes  # ['person', '0.7'] # 忽略某类

    # 设置根目录 make sure that the cwd() is the location of the python script (so that every path makes sense)
...
if __name__ == '__main__':
    ...
    MINOVERLAP = CFG.CONFIDENCE  # 置信度 default value (defined in the PASCAL VOC2012 challenge)

    '''自定义参数'''
    no_animation = not CFG.ANIMATION  # 是否动画
    no_plot = not CFG.PLOT_RES  # 是否日志 和画图
    quiet = CFG.CONSOLE_PINTER  # 控制台输出
    set_class_iou = CFG.IOU_MAP  # ['person', '0.7'] # 忽略某类
    ...
    f_do_fmap(CFG.PATH_GT, CFG.PATH_DT, CFG.PATH_IMG,
              confidence=CFG.CONFIDENCE,
              iou_map=CFG.IOU_MAP,
              ignore_classes=CFG.IGNORE_CLASSES,
              console_pinter=CFG.CONSOLE_PINTER,
              plot_res=CFG.PLOT_RES,
