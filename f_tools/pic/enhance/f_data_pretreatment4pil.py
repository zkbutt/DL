...
class Resize(object):
    ...
    def __init__(self, size_hw):
        ...
        :param size_hw: (h, w)
        '''
        self.size_hw_resize = torchvision.transforms.Resize(size_hw)
    ...
    def __call__(self, img_pil, target, cfg):
        ...
        h_ratio, w_ratio = np.array([h, w]) / self.size_hw_resize.size  # hw
        img_pil = self.size_hw_resize(img_pil)
        ...
        if random.random() < self.prob:
            ...
            if target:
                ...
                if 'keypoints' in target:
                    ...
                    mask = torch.all(keypoints > 0, dim=1)  # 全有效的行
                    # ids = np.nonzero(_t)  # 获取 需更新的行索引      6和8是0
                    # ids = torch.nonzero(_t)  # 获取 需更新的行索引      6和8是0
                    # ids = np.arange(0, len(keypoints[0]), 2)
                    # keypoints[ids, ::2] = width - keypoints[ids, ::2]
                    keypoints[mask, ::2] = 1.0 - keypoints[mask, ::2]
