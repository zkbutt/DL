...
class InvertedResidual(layers.Layer):
    def __init__(self, in_channel, out_channel, stride, expand_ratio, **kwargs):
        ...
        layer_list.extend([
            ...
            # 1x1 pointwise conv(linear) 这里是线性激活
...
def MobileNetV2(im_height=224, im_width=224, num_classes=1000, alpha=1.0, round_nearest=8):
    ...
    :param num_classes:
    :param alpha: 调整卷积核个数
    :param round_nearest: 输出的基数  确保输出核个数为 = n * round_nearest
    :return:
    '''
    # 调整核个数
    ...
    for t, c, n, s in inverted_residual_setting:
        ...
        for i in range(n):
            ...
            # 输入维度的最后一个（通道）
            x = InvertedResidual(x.shape[-1], output_channel, stride, expand_ratio=t)(x)
...
if __name__ == '__main__':
    net = MobileNetV2()
    print(net.summary())
    # print(plot_model(net, to_file='f_mobilenet_v2.png', show_shapes=True))
