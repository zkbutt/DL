...
def GoogLeNet(im_height=224, im_width=224, class_num=1000, aux_logits=False):
    ...
    x = cre_inception(x, 192, 96, 208, 16, 48, 64)
    if aux_logits:
        # aux1 = InceptionAux(class_num, name="aux_1")(x)
        aux1 = cre_inceptionaux(x, class_num)
...
def cre_inception(input, ch1x1, ch3x3red, ch3x3, ch5x5red, ch5x5, pool_proj):
    '''
    64, 96, 128, 16, 32, 32
    128, 128, 192, 32, 96, 64

    '''
    branch1 = layers.Conv2D(ch1x1, kernel_size=1, activation="relu")(input)

    x = layers.Conv2D(ch3x3red, kernel_size=1, activation="relu")(input)
    branch2 = layers.Conv2D(ch3x3, kernel_size=3, padding="SAME", activation="relu")(x)

    x = layers.Conv2D(ch5x5red, kernel_size=1, activation="relu")(input)
    branch3 = layers.Conv2D(ch5x5, kernel_size=5, padding="SAME", activation="relu")(x)

    x = layers.MaxPool2D(pool_size=3, strides=1, padding="SAME")(input)  # caution: default strides==pool_size
    branch4 = layers.Conv2D(pool_proj, kernel_size=1, activation="relu")(x)

    x = layers.concatenate([branch1, branch2, branch3, branch4], axis=3 )
    return x


def cre_inceptionaux(input, num_classes):
    x = layers.AvgPool2D(pool_size=5, strides=3)(input)
...
class InceptionAux(layers.Layer):
    def __init__(self, num_classes, **kwargs):
        ...
        self.averagePool = layers.AvgPool2D(pool_size=5, strides=3)  # 向下取整
